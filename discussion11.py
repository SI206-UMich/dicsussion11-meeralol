...
def add_pets_from_json(filename, cur, conn):
    ...
    for dict in json_data:
        for i in range(len(dict)):
            cur.execute("INSERT INTO Species (id,title) VALUES (?,?)",(i,dict[i]))

            cur.execute("INSERT INTO Patients (pet_id, name, species_id, age, cuteness, agressiveness) VALUES (i, json_data[i].name, json_data[i].species, json_data[i].age, json_data[i].cuteness, json_data[i].agressiveness)", (dict))
